...
export default function MapContainer({
  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      try {
        ...
        // Handle sketch create
        sketch.on("create", (event) => {
          if (event.state === "complete") {
            const poly = event.graphic.geometry as Polygon;
            const coords = poly.rings[0].map(coord => [coord[0], coord[1]]);
  ...
  useEffect(() => {
    ...
    // Clear existing polygons (but not drawing graphics)
    const graphics = polygonLayerRef.current.graphics.items;
    const toRemove = graphics.filter(g => 
      g.symbol && 
      g.symbol.type === "simple-fill" && 
      g.symbol.color[0] === 255 && 
      g.symbol.color[1] === 178
    );
    polygonLayerRef.current.removeMany(toRemove);
      ...
      {drawingMode && (
        <Card className="absolute bottom-4 left-1/2 transform -translate-x-1/2 p-3 shadow-lg z-50">
          <div className="flex items-center space-x-3">
            <p className="text-sm font-medium">Click the polygon tool (top-left) then click points on the map</p>
            <Button
              size="sm"
              variant="destructive"
              onClick={() => {
