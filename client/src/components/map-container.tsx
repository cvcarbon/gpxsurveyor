        ...
        import("esri-leaflet").then((esriLeaflet) => {
          ...
          layerConfigs.forEach((config, index) => {
            ...
            fetch(`${config.url}?f=json`)
              ...
              .then(serviceInfo => {
                console.log(`${config.name} service response:`, serviceInfo);
                
                // Check if token is required
                if (serviceInfo.error && serviceInfo.error.code === 499) {
                  console.log(`${config.name} requires authentication`);
                  alert(`${config.name} requires ArcGIS authentication. Please sign in to your account in the popup window.`);
                  window.open('https://www.arcgis.com/home/signin.html', '_blank', 'width=800,height=600');
                }
                
                // Create the feature layer regardless (will show auth prompts as needed)
                ...
                layer.on('requesterror', (error: any) => {
                  ...
                  if (error.code === 499 || error.code === 403 || error.code === 401) {
                    console.log(`Authentication required for ${config.name}`);
                    alert(`Please sign in to your ArcGIS account to access ${config.name} data.`);
                    window.open('https://www.arcgis.com/home/signin.html', '_blank', 'width=800,height=600');
                  }
                });
              ...
              .catch(error => {
                console.error(`Error with ${config.name} service:`, error);
                
                // Still create layer - authentication might be handled by browser
                const layer = esriLeaflet.featureLayer({
                  url: config.url,
                  style: {
                    color: config.color,
                ...
                layer.on('requesterror', (error: any) => {
                  console.log(`Authentication needed for ${config.name}`);
